...
const messages = {
    empty: "查詢沒有返回任何內容 The query returned nothing",
    executing: "執行查詢 Executing query...",
    invite: "運行 SQL 查詢以查看結果 Run SQL query to see the results",
    loading: "加載數據庫 Loading database...",
};
...
async function start(name, path) {
    ...
    if (!loadedDatabase) {
        ui.status.error(` 加載數據庫失敗 Failed to load database 由 from ${path}`);
        return false;
    }
...
function openUrl() {
    const url = prompt("輸入數據庫文件所在網址 Enter database file URL:", "https://path/to/database");
    if (!url) {
        return;
    }
    startFromUrl(url);
...
async function save() {
    ...
    ui.status.info("儲存 Saving...");
    ui.result.clear();
    const savedDatabase = await sqlite.save(database, query);
    if (!savedDatabase) {
        ui.status.error("保存數據庫失敗 Failed to save database");
        return;
    }
...
function showTables() {
    ...
    if (!tables.length) {
        ui.status.info("數據庫為空 Database is empty");
        return;
    }
...
function showWelcome() {
    ...
    let message = `<p>${messages.invite}<br> 或加載 demo 演示 or load the ${demo}.</p>`;
    message += `<p>隨時單擊 logo 以從頭開始。Click the logo anytime to start from scratch.</p>`;
    if (!gister.hasCredentials()) {
        const settings = action("visit", "settings", "settings");
        message += `<p> 訪問 Visit ${settings} 進行分享 to enable sharing.</p>`;
    }
    const about = action("visit", "關於 About SQLime", "about");
    message += `<p>${about}</p>`;
...
function showSaved(database) {
    ...
    const shareUrl = `<copy-on-click href="${window.location}" class="button">
        複製分享鏈接 copy share link</copy-on-click>`;

    const url = gister.getUrl(database.id);
    if (url) {
        const gistUrl = `<a href="${url}" target="_blank">gist</a>`;
        let message = `<p>✓ 另存為 Saved as ${gistUrl}</p>`;
        message += `<p>${shareUrl}</p>`;
        ui.status.info(message);
    } else {
        const settings = action("visit", "settings", "settings");
        let message = `<p>✓ 保存到公有雲。Saved to the public cloud. 訪問 Visit ${settings} 用於私人共享。for private sharing.</p>`;
        message += `<p> ${shareUrl}</p>`;
        ui.status.info(message);
    }
}
